...
void    incremental_make_root(bit_project*, bit_forward_project*);
void    incremental_index_project(bit_project*, bit_forward_project*);
void    incremental_index_possible_file(bit_project*, uintmax_t, bit_path*, char*);
void    incremental_index_recursive(bit_project*, bit_forward_project*, uintmax_t, bit_path*, char*);
void    incremental_apply_main_file(bit_project*, bit_forward_project*);
void    incremental_apply_compilers(bit_project*, bit_forward_project*);
void    incremental_apply_linker_flags(bit_project*, bit_forward_project*);

int     incremental_compile_file(bit_project*, bit_file*);
int     incremental_link_executable(bit_project*, char*);
int     incremental_compile_with_absolute_path(bit_project*, char*);

void    incremental_mark_changed_files(bit_project*, bit_set*);
int     incremental_cached_compilation(bit_project*, bit_file*);
int     incremental_compile_changed_files(bit_project*, bit_set*, int*);

bit_project project_resolve(bit_forward_project* fproject) {
    bit_project project;
    bit_iter iter;
    bit_file* file;

    project.base = fproject->base;
    project.files = set_new(sizeof(bit_file));
...
void incremental_apply_cache(bit_project* project) {
    bit_iter iter;
    bit_file *file, *cached;
...
void incremental_index_possible_file(bit_project* project, uintmax_t parent_id, bit_path* path, char* name) {
    int exists;
    char* file_ending;
    bit_path file_path;
    bit_file file, *parent;
...
void incremental_index_recursive(bit_project* project, bit_forward_project* forward_project, uintmax_t parent_id, bit_path* path, char* name) {
    char *str_path, *file_name;
    uintmax_t directory_id;
    bit_path dir_path, sub_path;
    bit_os_dir* dir;
    bit_os_file* file_ptr;
    ...
    while ((file_ptr = os_dir_read(dir)) != NULL) {
        ...
        if (set_has(&forward_project->ignore_paths, os_file_id(file_ptr))) {
            log_debug("Ignoring: \"%s" BIT_PATH_SEP "%s\"", path_to_string(path), file_name);
            continue;
        }
...
void incremental_index_project(bit_project* project, bit_forward_project* forward_project) {
    bit_os_dir* dir;
    char* name;
    bit_path *path, extra_path;
    bit_iter iter;
...
void incremental_make_root(bit_project* project, bit_forward_project* fproject) {
    int exists;
    bit_path path = path_copy(&project->base.root);
    bit_file root = file_dir_new(&path, ".");
    root.build_info.compiler_set = 1;
    root.build_info.compiler.type = BIT_COMPILER;
...
void incremental_apply_main_file(bit_project* project, bit_forward_project* fproject) {
    int match_found = 0;
    bit_iter iter;
    bit_path path;
    bit_file* file;
...
void incremental_apply_compilers(bit_project* project, bit_forward_project* fproject) {
    size_t index;
    bit_iter name_iter, compiler_iter;
    bit_string* file_name;
...
void incremental_apply_linker_flags(bit_project* project, bit_forward_project* fproject) {
    size_t index;
    bit_iter name_iter, linker_flags_iter;
    bit_string* file_name;
...
int incremental_compile_file(bit_project* project, bit_file* file) {
    int result;
    char name[FILENAME_MAX];
    bit_string cmd = string_new();
    bit_path path;
    bit_string* executable;
    bit_array compiler_flags;
...
int incremental_link_executable(bit_project* project, char* executable_name) {
    int result;
    char name[FILENAME_MAX];
    bit_path path;
    bit_file *main_file, *file;
    bit_string cmd;
    bit_array linker_flags;
    bit_iter iter;
...
void incremental_mark_changed_files(bit_project* project, bit_set* changed_files) {
    int* has_changed;
    bit_file *file, *cache_file, *temp;
    bit_iter iter;
...
int incremental_cached_compilation(bit_project* project, bit_file* file) {
...
int incremental_compile_with_absolute_path(bit_project* project, char* name) {
    bit_iter iter;
    int result = 0, any_compiled = 0, temp;
    bit_path path;
    bit_file *file;
    bit_set changed_files;
...
int incremental_compile_changed_files(bit_project* project, bit_set* changed_files, int* any_compiled) {
    FILE* cached_file;
    int *has_changed, result, temp;
    char compiled_name[FILENAME_MAX];
    bit_iter iter;
    bit_path path;
    bit_string compiled_path;
    bit_file* file;

    result = 0;
    iter = iter_set(&project->files);
    while (iter_next(&iter, (void**) &file)) {
        if (file->type == BIT_HEADER || file->type == BIT_DIR) {continue;}
...
int incremental_compile_project(bit_project* project, char* name) {
    int result;
    bit_path executable_path = path_copy(&project->base.root);
...
int incremental_test_project(bit_project* project, char* path) {
